...
func _ready():
	...
	_buildSprites(textures)

	if textures.size() == 0:
		print("Error: No textures available.")
		return

	var random = RandomNumberGenerator.new()
	random.randomize()
	var index_sorted = random.randi_range(0, textures.size() - 1)

	if index_sorted < 0 or index_sorted >= textures.size():
		print("Error: Invalid index_sorted value.")
		return

	var path = textures[index_sorted]
	image.load(path)
...
func scramble_board():
	...
	if not is_board_solvable(temp_flat_board):
		randomize()
		temp_flat_board.shuffle()
...
func _buildSprites(textures):
	var dir = "res://assets/backgrounds"
	var files = Directory.new()

	files.open(dir)
	files.list_dir_begin(true)
